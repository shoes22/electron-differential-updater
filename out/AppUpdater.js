...
function _prepareAppZip() {
  const data = require("./prepareAppZip");

  _prepareAppZip = function () {
    return data;
  };
...
class AppUpdater extends _events().EventEmitter {
  ...
  checkForUpdatesAndNotify(downloadNotification) {
    ...
    return this.checkForUpdates().then(it => {
      ...
      downloadPromise.then(() => {
        const notificationContent = this.formatDownloadNotification(it ? it.updateInfo.version : "", this.app.name, downloadNotification);
  ...
  async getOrCreateDownloadHelper(useAppSupportCache) {
    ...
    if (result == null) {
      ...
      const cacheDir = path.join(useAppSupportCache ? this.getAppSupportCacheDir() : this.app.baseCachePath, dirName || this.app.name);
  ...
  async executeDownload(taskOptions) {
    ...
    const downloadedUpdateHelper = await this.getOrCreateDownloadHelper(useAppSupportCache);
...
function emitDownloadProgress(DOWNLOAD_PROGRESS, arg1) {// this.emit(DOWNLOAD_PROGRESS, arg1);
}
