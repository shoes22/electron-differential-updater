  ...
  async differentialDownloadInstaller(fileInfo, downloadUpdateOptions, installerPath, provider) {
    try {
      ...
      const getBlockMapUrl = () => {
        if (fileInfo.info.url.includes(`${downloadUpdateOptions.updateInfoAndProvider.info.version}`)) {
          return `${fileInfo.url.pathname}.blockmap`;
        } else {
          const {
            app
          } = this.app;
          return `${fileInfo.url.pathname.replace(new RegExp(fileInfo.info.url, "g"), `${app.productName || app.name} Setup ${downloadUpdateOptions.updateInfoAndProvider.info.version}.exe`)}.blockmap`;
        }
      };

      const blockMapUrl = getBlockMapUrl();
      const newBlockMapUrl = (0, _main().newUrlFromBase)(blockMapUrl, fileInfo.url);
      const oldBlockMapUrl = (0, _main().newUrlFromBase)(`${blockMapUrl.replace(new RegExp(downloadUpdateOptions.updateInfoAndProvider.info.version, "g"), this.app.version)}`, fileInfo.url);
