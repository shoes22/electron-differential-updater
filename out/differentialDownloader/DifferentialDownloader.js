...
class DifferentialDownloader {
  ...
  doDownload(oldBlockMap, newBlockMap, emit) {
  ...
  downloadFile(tasks, emit) {
    ...
    return this.doDownloadFile(tasks, fdList, emit).then(closeFiles).catch(e => {
  ...
  async doDownloadFile(tasks, fdList, emit) {
    let oldFileFd;
    oldFileFd = await (0, _fsExtra().open)(this.options.oldFile, "r");
    fdList.push({
    ...
    await new Promise((resolve, reject) => {
      ...
      const firstStream = streams[0]; // TASK - use useMultipleRangeRequest property from package.json
